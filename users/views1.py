...
def showmessage(request):
    ...
    for row in data:
        usermovieid.append(row.doubanid)
    ...
    try:
        ...
        for i in usermovieid:
            cur.execute('select * from moviegenre3 where doubanid = %s', i)
            rr = cur.fetchall()
            for doubanid, title, poster in rr:
...
def recommend2(request):            # userbased
    ...
    Insertposter.objects.filter(userId=USERID).delete()
    # selectMysql()
    read_mysql_to_csv('users/static/douban_filter.csv', USERID)  # 追加数据，提高速率
    ratingfile = os.path.join('users/static', 'douban_filter.csv')
    usercf = UserBasedCF()
...
def recommend1(request):        # itembased
    ...
    itemcf.recommend(userid)  # 得到doubanId号
...
def insert(request):
    ...
    doubanid = int(request.GET["doubanid"])

    Resulttable.objects.create(userId=USERID, rating=RATING, doubanid=doubanid)
    # print(USERID)
    # return HttpResponseRedirect('/')
    return render(request, 'index.html', {'userId': USERID,'rating': RATING, 'doubanid': doubanid })
...
def read_mysql_to_csv(filename, user):
    with codecs.open(filename=filename, mode='w', encoding='utf-8') as f:
        ...
        conn = get_conn()
        cur = conn.cursor()
        cur.execute('select * from douban_filter')
        # sql = ('select * from douban_filter WHERE userId = 100001')
        rr = cur.fetchall()
...
class UserBasedCF(object):
    ...
    def recommend(self, user):
        ...
        for similar_user, similarity_factor in sorted(user_sim_mat[user].items(),
            ...
            for doubanid in self.trainset[similar_user]:  # similar_user是items里面的键，就是所有用户   similarity_factor是值，就是对应的相似度                # predict the user's "interest" for each movie
                if doubanid in watched_movies:
                    continue
                rank.setdefault(doubanid, 0)  # 没有值就为0
                rank[doubanid] += similarity_factor  # rank[movie]就是各个电影的相似度
        ...
        for key, value in rank_:
            matrix.append(key)  # matrix为存储推荐的doubanId号的数组
            # print(key)     #得到了推荐的电影的doubanid号
        # print(matrix)
...
class ItemBasedCF(object):
    ...
    def recommend(self, user):
        ...
        for key, value in rank_:
            matrix2.append(key)  # matrix为存储推荐的doubanId号的数组
            # print(key)     #得到了推荐的电影的doubanid号
        # print(matrix2)
        return matrix2
...
if __name__ == '__main__':
    ratingfile2 = os.path.join('static', 'users_resulttable2.csv')  # 一共671个用户

    usercf = ItemBasedCF()
    userId = '1'
    # usercf.initial_dataset(ratingfile1)
    usercf.generate_dataset(ratingfile2)
    usercf.calc_movie_sim()
    # usercf.evaluate()
