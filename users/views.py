...
from users.models import Resulttable, Insertposter, Mysearch
...
global tmpmatrix
tmpmatrix = np.load('my_file.npy', allow_pickle=True).item()
...
def showmessage(request):
    ...
    usermovietitle = []
    userrate=[]
    data = Resulttable.objects.filter(userId=100001)
    for row in data:
        usermovieid.append(row.imdbId)
        userrate.append(row.rating)
    ...
    try:
        ...
        for i in usermovieid:
            ...
            rr = cur.fetchall()
            for imdbId, title, poster, link in rr:
    ...
    # 合并
    lst = []
    length = len(usermovietitle)
    for i in range(length):
        lst.append(usermovietitle.pop(0))
        lst.append(userrate.pop(0))
    return render(request, 'users/message.html', locals())


# USERID = 1002
def recommend2(request):  # userbased
    # USERID = int(request.GET["userIdd"]) + 100000
    # Insertposter.objects.filter(userId=USERID).delete()
    # # selectMysql()
    ...
    # usercf.recommend(userid)  # 得到doubanid号

    USERID = int(request.GET["userIdd"]) + 100000
    Insertposter.objects.filter(userId=USERID).delete()
    # selectMysql()
    read_mysql_to_csv('users/static/users_resulttable.csv', USERID)  # 追加数据，提高速率
    ratingfile = os.path.join('usersatic', 'users_resulttable.csv')
    usercf = UserBasedCF()
    userid = str(USERID)  # 得到了当前用户的id
    print(userid)
    # usercf.generate_dataset(ratingfile)
    # usercf.calc_user_sim()
    usercf.yzqFunc()  # 得到imdbId号
...
def recommend1(request):  # itembased
    # USERID = int(request.GET["userIddd"]) + 100000
    USERID = 100001
    Insertposter.objects.filter(userId=USERID).delete()
    # selectMysql()
...
def search(request):
    ...
    i = str(request.GET["id_search"])

    print("the receive input is:")

    sql = "select * from moviegenre3 where title like '%" + i + "%'"
    print(sql)
    try:
        conn = get_conn()
        cur = conn.cursor()
        ...
        j = cur.fetchall()
        print(j)
        for imdbId, title, poster, link in j:
            # print(value)         #value才是真正的海报链接
            if (Mysearch.objects.filter(imdbId=imdbId)):
                continue
            else:
                Mysearch.objects.create(title=title, poster=poster, imdbId=imdbId, link=link)

        # print(poster_result)
    finally:
        conn.close()

        results = Mysearch.objects.all()
    return render(request, 'users/movieRecommend3.html', locals())


def insert(request):
    # MOVIEID = int(request.GET["movieId"])
    global USERID
    USERID = int(request.GET["userId"]) + 100000
    # USERID = {{}}
...
def get_conn():
    conn = pymysql.connect(host='localhost', port=3306, user='root', passwd='123456', db='moviedata',
                           charset='utf8')
    return conn
...
def read_mysql_to_csv(filename, user):
    with codecs.open(filename=filename, mode='w', encoding='utf-8') as f:
        ...
        # sql = ('select * from users_resulttable WHERE userId = 100001')
        rr = cur.fetchall()
...
def read_mysql_to_csv2(filename, user):
    with codecs.open(filename=filename, mode='w', encoding='utf-8') as f:
        ...
        conn = get_conn()
        cur = conn.cursor()
        cur.execute('select * from users_resulttable WHERE userId = 100001')
...
class UserBasedCF(object):
    ...
    #         for imdbid in self.trainset[similar_user]:  # similar_user是items里面的键，就是所有用户   similarity_factor是值，就是对应的相似度                # predict the user's "interest" for each movie
    ...
    def filtered(self, sent):
        return [item for item in sent if item[0] == 100001]
    ...
    def get_top_n(self, predictions, n=10):
        ...
        for uid, user_ratings in top_n.items():
            if uid == 100001:
                ...
                top_n[uid] = [x for x, _ in user_ratings[:n]]
...
class ItemBasedCF(object):
    ...
    def calc_movie_sim(self):
        ...
        #
        # #save to txt
        # np.save('my_file.npy', itemsim_mat, allow_pickle=True)
        # print("okk!!!!!!!!!")
        #
        # 测试
        # self.itemsim_mat = np.load('my_file.npy', allow_pickle=True).item()
        # print(self.itemsim_mat['1294362'])
        # print("okk2")
        # time.sleep(200)
    ...
    def recommend(self, user):
        ...
        # self.tmpmatrix = np.load('my_file.npy', allow_pickle=True).item()
        # print("okk")

        for movie, rating in watched_movies.items():
            for related_movie, similarity_factor in sorted(tmpmatrix[movie].items(),
